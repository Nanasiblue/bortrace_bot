...
def strategy_summary(picked: pd.DataFrame) -> dict[str, Any]:
    if picked.empty:
        return {
            "races": 0,
            "bets": 0,
...
def pick_by_rule(scored: pd.DataFrame, *, top_per_race: int, min_odds: float, max_odds: float, min_model_ev: float) -> pd.DataFrame:
    picked = scored[
        (scored["odds"] >= min_odds)
        & (scored["odds"] <= max_odds)
        & (scored["model_ev"] >= min_model_ev)
    ].copy()
    if picked.empty:
        return picked
...
def main() -> None:
    ...
    parser.add_argument("--candidate-model", default=str(OUTPUT_DIR / "candidate_lightgbm_2023_2025_valid_202607" / "lightgbm.pkl"))
    parser.add_argument("--out-dir", default=str(OUTPUT_DIR / "live_candidate_eval_202601_202607"))
    ...
    for idx, race in df.iterrows():
        if args.progress_every and (idx + 1) % args.progress_every == 0:
            print(f"races={idx + 1}/{len(df)} candidates={len(rows)}", flush=True)
    ...
    for top_per_race in [1, 2, 3, 5]:
        for min_model_ev in [-0.25, -0.1, 0.0, 0.05, 0.1, 0.2, 0.35, 0.5, 0.8, 1.0]:
            for min_odds, max_odds in [(1, 50), (1, 80), (1, 120), (20, 100), (20, 150), (50, 150), (50, 300), (100, 500)]:
                picked = pick_by_rule(
                    ...
                    min_odds=min_odds,
                    max_odds=max_odds,
                    min_model_ev=min_model_ev,
                )
                summary = strategy_summary(picked)
                summary.update(
                    {
                        "top_per_race": top_per_race,
                        "min_model_ev": min_model_ev,
    ...
    summary_df = pd.DataFrame(summaries).sort_values(
        ["roi", "hits", "zero_hit_days", "bets"], ascending=[False, False, True, False]
    )
    summary_df.to_csv(out_dir / "strategy_summary.csv", index=False, encoding="utf-8")
    candidates.nlargest(50000, "model_ev").to_csv(out_dir / "scored_top.csv", index=False, encoding="utf-8")
    ...
    metrics = {
        ...
        "best_strategy": summary_df.head(1).to_dict(orient="records")[0] if not summary_df.empty else {},
    }
